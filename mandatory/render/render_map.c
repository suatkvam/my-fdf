...
/*   Updated: 2025/10/16 18:45:39 by akivam           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	render_point_connections(t_render *render, int x, int y)
{
	...
	current_2d = project_point_scaled(current_point, render->map);
	if (x < render->map->width - 1)
	{
		right_2d = project_point_scaled(render->map->grid[y][x + 1],
				render->map);
		draw_line_hybrid(render, current_2d, right_2d);
	}
	if (y < render->map->height - 1)
	{
		down_2d = project_point_scaled(render->map->grid[y + 1][x],
				render->map);
